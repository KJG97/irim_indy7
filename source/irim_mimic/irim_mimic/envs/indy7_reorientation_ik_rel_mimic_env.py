...
class INDY7ReorientationIKRelMimicEnv(ManagerBasedRLMimicEnv):
    """
    ...
    def get_subtask_term_signals(self, env_ids: Sequence[int] | None = None) -> dict[str, torch.Tensor]:
        ...
        signals["touch"] = subtask_terms["touch"][env_ids]
        # signals["center"] = subtask_terms["center"][env_ids]
    ...
    def actions_to_gripper_actions(self, actions):
        """
        Indy7 로봇에는 그리퍼가 없으므로 빈 딕셔너리를 반환합니다.
        """
        # eef_name을 가져와서 빈 그리퍼 액션 텐서를 반환
        gripper_actions = {}
        for eef_name in self.cfg.subtask_configs.keys():
            # 빈 그리퍼 액션 텐서 생성 (액션과 같은 길이의 0 텐서)
            gripper_actions[eef_name] = torch.zeros(actions.shape[0], 1, device=self.device)
